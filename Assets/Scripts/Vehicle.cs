...
namespace Aether {
 public partial class Vehicle:MonoBehaviour {
  ...
  public void Initialize(Game g,int index,bool isPlayer,string name,Vector3 p,Quaternion q,int tune=0,int tires=0,int brakes=0){
   ...
   PremiumLighting.Headlights(transform,player,track.map);body.maxAngularVelocity=16;lastPoint=track.Nearest(p);progress=track.isRush?0:lastPoint>TrackWorld.Count/2?lastPoint-TrackWorld.Count:lastPoint;lastSafe=p;safeRot=q;CarParts.Apply(model,isPlayer?g.CurrentCar:new SavedCar{body=index,engineLevel=tune,tireLevel=tires,brakeLevel=brakes});
  }
  ...
  void DriveAI(){
   ...
   float bend=Vector3.Angle(track.Forward(near),track.Forward(track.isRush?Mathf.Min(near+ahead*2,TrackWorld.Count-1):(near+ahead*2)%TrackWorld.Count));
   ...
   energy=Mathf.Min(100,energy+Time.deltaTime*(drift?16:7));if(BoostAvailable&&safeBoost&&energy>25&&speed<desired+12){boost=Mathf.Max(boost,.12f);energy=Mathf.Max(0,energy-Time.deltaTime*27*(player&&game.inventory.engine=="storm-coil"?.8f:1));}
